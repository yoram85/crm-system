...
const Login = () => {
  ...
  const { login, signInWithGoogle, signInWithFacebook, isAuthenticated } = useAuthStore()
  ...
  // Check for OAuth hash in URL (callback from Google/Facebook)
  useEffect(() => {
  ...
  const handleFacebookSignIn = async () => {
    ...
    try {
      ...
      setError(`שגיאה בהתחברות עם Facebook: ${err?.message || 'נסה שוב'}`)
      setIsLoading(false)
    }
  }
          ...
          {showEmailForm ? (
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-blue-600 text-white py-4 rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-3"
              >
                <Mail size={20} />
                <span className="text-lg">התחבר עם אימייל</span>
              </button>
              ...
              >
                ...
                <span className="text-lg">התחבר עם Facebook</span>
              </button>
